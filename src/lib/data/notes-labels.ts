// Display labels + descriptions for the notes tracks, in Bangla. Keyed by the
// same category slug as CATEGORIES (categories.ts). Track *identity* (slug,
// group) stays English-defined — and the English labels still drive the
// /directory page — so only what the notes render lives here.
import { CATEGORIES, type CategoryMeta } from './categories';

type LocalizedCategory = { label: string; description: string };

export const NOTES_CATEGORIES: Record<string, LocalizedCategory> = {
...
/** Category metadata with the notes label/description where one exists. */
export function categoryMeta(key: string): CategoryMeta | undefined {
	const base = CATEGORIES[key];
	if (!base) return undefined;
	const local = NOTES_CATEGORIES[key];
	return local ? { ...base, label: local.label, description: local.description } : base;
}

/** Just the display label for a category (falls back to the slug). */
