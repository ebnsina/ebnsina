	...
	<div class="mb-7 flex flex-wrap items-end justify-between gap-4">
		<div>
			...
			<p class="mt-2 max-w-xl text-sm text-muted">
				Four levels, in order. Start at the top and work down — or jump anywhere you like.
			</p>
		</div>
		{#if !allDone && next}
			<a
			...
			<li class="flex gap-4 sm:gap-5">
				...
				<div class="relative flex w-9 shrink-0 flex-col items-center">
					<span
						...
						style="color: {LEVEL_COLOR[lvl.level]}; background: color-mix(in oklch, {LEVEL_COLOR[lvl.level]} 16%, transparent); {pct === 100 ? `background: ${LEVEL_COLOR[lvl.level]}; color: #fff;` : ''}"
					>
						{lvl.n}
					</span>
					{#if i < levels.length}
				...
				<!-- stage -->
				<div
					class="mb-4 flex-1 rounded-2xl border p-5 transition-colors"
				...
				>
					<div class="flex flex-wrap items-center gap-x-3 gap-y-2">
						<span class="font-pixel text-[0.65rem] uppercase tracking-wider text-muted"
							>Level {lvl.n}</span
						>
					...
					<p class="mt-2 max-w-2xl text-sm leading-relaxed text-muted">{lvl.blurb}</p>
