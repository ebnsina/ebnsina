...
export function buildLadder(
	...
	for (const reference of REFERENCE_BITRATES) {
		...
		const finalBitrate =
			bpp > MAX_BPP
				? Math.round(MAX_BPP * candidate.width * candidate.height * fps)
				: bitrate;
...
export function ffmpegArgs(
	...
	return [
		...
		'-s', `${rung.width}x${rung.height}`,
		'-r', String(rung.fps),
		'-b:v', String(rung.bitrate),
		...
		'-g', String(rung.gopFrames),
		'-keyint_min', String(rung.gopFrames),
		'-sc_threshold', '0',
