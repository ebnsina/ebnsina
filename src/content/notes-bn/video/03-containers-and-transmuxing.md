...
কায়রোর একটা প্রেরণ-ঘর, যেখান থেকে গোটা সাম্রাজ্যে পাণ্ডুলিপি পাঠানো হয়। এর প্রধান ইবনে আল-হাইসাম। তাঁর ঘরে দুটো আলাদা কাজ হয় — লেখা আর মোড়ানো — আর তিনি নবীন কর্মচারীদের প্রথম দিনেই এই পার্থক্যটা বুঝিয়ে দেন।
...
এখান থেকেই ইবনে আল-হাইসামের বিখ্যাত ভুলটা হয়েছিল। শুরুর দিকে তাঁর নিয়ম ছিল — কাগজ আগে ভরো, তারপর গুনে গুনে তালিকা বানিয়ে **বাক্সের নিচে** সেঁটে দাও। যুক্তিসঙ্গত, কারণ সব ভরার আগে তো তালিকা লেখা যায় না। কিন্তু কর্ডোবার গ্রন্থাগারিক চিঠি লিখলেন: "আপনার উটের কাফেলা বাক্স নিয়ে আসে ধীরে ধীরে। আমি চাই প্রথম বস্তা পৌঁছানো মাত্র পড়া শুরু করতে। কিন্তু তালিকা তো সবার শেষে আসে — মানে গোটা কাফেলা না পৌঁছানো পর্যন্ত আমি কিছুই খুলতে পারছি না।" ইবনে আল-হাইসাম নিয়ম বদলালেন: এখন বাক্স ভরার পর তালিকাটা খুলে **সামনে এনে** সাঁটা হয়। কাজটা একটু বাড়তি, কিন্তু এখন প্রথম বস্তাতেই গ্রন্থাগারিক জানে ভেতরে কী আছে আর কোথায় আছে।
...
আর সবচেয়ে গুরুত্বপূর্ণ ঘটনাটা ঘটল একদিন সকালে। কর্ডোবার গ্রন্থাগার বলল তারা কাঠের সিন্দুক নিতে পারবে না, শুধু চামড়ার থলে নেবে। ইবনে আল-হাইসামের সহকারী আল-রাযি হতাশ হয়ে বলল — "তাহলে গোটা পাণ্ডুলিপি আবার নকল করাতে হবে!" ইবনে আল-হাইসাম হেসে বললেন, "কেন? লিপি তো বদলাচ্ছে না। কাগজগুলো সিন্দুক থেকে বের করো, থলেতে ভরো, নতুন তালিকা লেখো। ছয় মাসের বদলে দুই ঘণ্টার কাজ।" কাগজে একটা আঁচড়ও পড়ল না। **শুধু তখনই এটা চলে যখন গ্রন্থাগার লিপিটা পড়তে পারে** — কেউ যদি বলত "আমাদের এখানে কেউ শর্টহ্যান্ড পড়তে জানে না", তখন আর মোড়ক বদলে কাজ হতো না, সত্যিই নতুন করে লিখতে হতো।
...
async function parseTimescaleBox(
	...
	const payload = await readAt(handle, box.offset + box.headerSize, Math.min(box.size - box.headerSize, 32));
...
async function parseSampleDescription(handle: FileHandle, box: Box): Promise<string> {
	const payload = await readAt(handle, box.offset + box.headerSize, Math.min(box.size - box.headerSize, 24));
...
async function inspect(path: string): Promise<FileReport> {
	...
	try {
		...
		const fragmented = Boolean(findBox(moov.children, 'mvex')) || Boolean(findBox(topLevel, 'moof'));
...
function printReport(report: FileReport): void {
	...
	for (const track of report.tracks) {
		...
		console.log(`    duration      ${track.durationSeconds.toFixed(2)}s @ timescale ${track.timescale}`);
