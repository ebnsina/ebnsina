...
class LiveMediaPlaylist {
	...
	render(): string {
		...
		this.segments.forEach((segment, position) => {
			if (position >= partFloor) {
				for (const part of segment.parts) {
					const attrs = [
						`DURATION=${part.durationSeconds.toFixed(3)}`,
						`URI="${part.uri}"`
					];
		...
		if (open && !open.complete) {
			...
			const hinted =
				`${this.config.basePath}seg-${String(open.msn).padStart(5, '0')}.${nextIndex}.m4s`;
