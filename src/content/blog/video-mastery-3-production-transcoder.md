...
```text
      ব্রাউজার
         │  (১) সরাসরি আপলোড, presigned URL
         ▼
   অবজেক্ট স্টোরেজ ──(২) ঘটনা──► API ──(৩) probe ──► ডেটাবেস
                                    │
                                    │ (৪) কাজ ঢোকানো
                                    ▼
                                  সারি
              ...
              ┌─────────────┬───────┴───────┬─────────────┐
              ▼             ▼               ▼             ▼
          ওয়ার্কার ১    ওয়ার্কার ২     ওয়ার্কার ৩    ওয়ার্কার N
              └─────────────┴───────┬───────┴─────────────┘
                                    │ (৫) আউটপুট
                                    ▼
                            অবজেক্ট স্টোরেজ
...
```text
uploaded → probing → queued → encoding → packaging → publishing → ready
              │                   │           │
...
```text
              ┌─► থাম্বনেইল + স্প্রাইট ─┐
probe ─► চাংক ─┼─► এনকোড চাংক ১ ─┐      │
              │   এনকোড চাংক ২ ─┼─► জোড়া ─┼─► প্যাকেজ ─► প্রকাশ
              │   এনকোড চাংক N ─┘      │
              └─► সাবটাইটেল ───────────┘
```
