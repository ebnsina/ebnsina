...
বিরুনি একটা ছোট কোর্স প্ল্যাটফর্ম বানাচ্ছে। প্রথম শিক্ষক হিসেবে সিনা একটা লেকচার আপলোড করল — ৪২ মিনিটের একটা ফাইল, ২.৩ গিগাবাইট, ফোন থেকে রেকর্ড করা।

বিরুনি ফাইলটা সার্ভারে রেখে দিল, আর পেজে একটা সাধারণ ভিডিও ট্যাগ বসিয়ে দিল। কাজ শেষ, ভাবল সে।
...
<Mermaid
title="ffmpeg-এর ভেতরের পাইপলাইন"
code={`flowchart LR
  I["ইনপুট ফাইল"] --> DM["ডিমাক্স<br/>খাম খোলা"]
  DM --> DC["ডিকোড<br/>কাঁচা ফ্রেম"]
  DC --> F["ফিল্টার<br/>রিসাইজ, ঘোরানো"]
  F --> EN["এনকোড<br/>আবার চাপা"]
  EN --> MX["মাক্স<br/>খামে ভরা"]
  MX --> O["আউটপুট"]
  DM -.->|"রিমাক্স — ডিকোড ছাড়াই"| MX`}
/>
...
বিরুনির ফাইলে দুটো লাল পতাকা: ফ্রেমরেট পরিবর্তনশীল, আর ঘোরানোর তথ্য আছে। দুটোই এখনই ঠিক করে নেব।
...
- **`-pix_fmt yuv420p`** — এটাই বিরুনির "আইফোনে কালো স্ক্রিন" সমস্যার সমাধান। ইনপুট 4:2:2 বা 4:4:4 হলে আউটপুটও তাই হয়ে যেত, আর বহু ডিভাইস সেটা চালাতে পারে না।
...
বিরুনির ২.৩ গিগার ফাইল এই এক কমান্ডেই নেমে এলো ৩৮০ মেগাবাইটে। কিন্তু কাজ শেষ না — এটা এখনো একটাই ফাইল, একটাই কোয়ালিটি।
...
এবার ধাপগুলো। বিরুনির কনটেন্ট মূলত লেকচার — স্লাইড আর কথা বলা মানুষ — তাই বিটরেট তুলনামূলক কম রাখা যায়।
...
দেখতে ভয়ংকর, কিন্তু ভেতরে চেনা জিনিসই — একবার ডিকোড, চারবার এনকোড:

<Mermaid
title="এক পাসে পুরো ল্যাডার"
code={`flowchart LR
  I["ইনপুট"] --> D["ডিকোড<br/>একবারই"]
  D --> SP{"split=4"}
  SP --> R1["scale 360<br/>crf 26 · 700k"]
  SP --> R2["scale 480<br/>crf 24 · 1400k"]
  SP --> R3["scale 720<br/>crf 23 · 3000k"]
  SP --> R4["scale 1080<br/>crf 22 · 6000k"]
  R1 --> H["HLS প্যাকেজার<br/>২ সেকেন্ডের fMP4"]
  R2 --> H
  R3 --> H
  R4 --> H
  H --> M["master.m3u8<br/>+ চারটা ফোল্ডার"]`}
/>
...
হাতে কমান্ড চালানো শেখার জন্য ভালো, কিন্তু বিরুনির দরকার এমন কিছু যেটা আপলোড হলেই নিজে কাজটা করে।

<Mermaid
title="ওয়ার্কারের জীবনচক্র"
code={`flowchart TD
  W["inbox-এ নজর"] --> Q["সারিতে যোগ"]
  Q --> C{"এখন কি<br/>কিছু চলছে?"}
  C -->|"হ্যাঁ"| WAIT["অপেক্ষা"]
  WAIT --> C
  C -->|"না"| P["probe"]
  P --> LAD["ইনপুট অনুযায়ী<br/>ল্যাডার ঠিক করা"]
  LAD --> E["ffmpeg — এনকোড ও প্যাকেজ"]
  E --> T["থাম্বনেইল ও স্প্রাইট"]
  T --> OK["তখনই 'রেডি' চিহ্ন"]
  P --> FAIL["ব্যর্থ"]
  E --> FAIL
  OK --> CL["temp মুছে ফেলা"]
  FAIL --> CL
  CL --> C`}
/>
...
বিরুনির জিনিসটা এখন কাজ করে। সিনা আপলোড করে, কিছুক্ষণ পর সব ডিভাইসে ভিডিও চলে — এমনকি সমরকন্দের দুর্বল নেটেও।
