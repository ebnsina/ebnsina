...
level: "intermediate"
readingTime: "11 min"
topics: ["shard routing", "connection management", "transactions", "migration", "TypeScript", "PostgreSQL"]
---
...
class OrderService {
  async placeOrder(customerId: string, items: OrderItem[]): Promise<Order> {
    ...
    try {
      ...
      await client.query('UPDATE orders SET status = $1 WHERE id = $2', ['confirmed', order.rows[0].id]);
...
async function transferCredits(
  ...
  if (debitResult.rows.length === 0) {
    await fromShard.query(
      'UPDATE credit_transfers SET status = $1 WHERE id = $2',
      ['failed_insufficient_funds', transferId]
    );
  ...
  try {
    ...
    await fromShard.query(
      'UPDATE credit_transfers SET status = $1 WHERE id = $2',
      ['failed_credit_error', transferId]
    );
    throw err;
  }
}
...
async function migrateShard(fromShardId: string, toShardId: string) {
  ...
  while (true) {
    const rows = await fromPool.query(
    ...
    const toMigrate = rows.rows.filter(row =>
      ring.getShard(row.customer_id) === toShardId
    );
